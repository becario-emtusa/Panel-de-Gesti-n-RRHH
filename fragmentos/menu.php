<link rel="stylesheet" href="/dist/css/menu.css">

<section class="section-emtusa">
    <nav>
        <div class="logo-1">
            <img class="logo-emtusa" src="/images/emtusa-logo-cab.png" alt="logoEmtusa.png">
        </div>
        <ul class="nav-links">
            <li><a href="/inicio" class="nav-link"><i class="fas fa-home"></i>Inicio</a></li>
            <li><a href="/solicitudes" class="nav-link"><i class="fas fa-edit"></i>Realizar Solicitud</a></li>
            <li><a href="/consultar-solicitudes" class="nav-link"><i class="fas fa-search"></i>Consultar Solicitudes</a></li>
        </ul>
        <div class="burguer">
            <div class="line1"></div>
            <div class="line2"></div>
            <div class="line3"></div>
        </div>
        <div class="logo-2">
            <img class="logo-ayuntamiento" src="/images/ayuntamiento-huelva-logo-cab.png" alt="logoAyuntamiento.png">
        </div>
    </nav>
</section>